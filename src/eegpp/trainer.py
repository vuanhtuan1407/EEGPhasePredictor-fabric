...
class EEGKFoldTrainer:
    def __init__(
        ...
        #     self.models[i], self.optimizers[i] = self.fabric.setup(self.models[i], self.optimizers[i])
    ...
    def base_step(self, model, batch_idx, batch):
        ...
        lb_binary = lb_binary.to(self.device)
            ...
            for epoch in range(self.n_epochs):
                ...
                with torch.no_grad():
                    ...
                    for batch_idx, batch in val_bar:
                        ...
                        loss = self.loss_fn(pred[:, params.POS_IDX, :-1], lb[:, params.POS_IDX, :-1])
